...
@section('content')
    <section class="content-header">
        <h1>Poster Diri <small>Peserta</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-home"></i> Beranda</a></li>
            <li>Peserta</li>
            <li class="active">Poster Diri</li>
        </ol>
    </section>
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="box box-default">
                    <form method="post" action="{{ action('Mahasiswa\PosterDiriController@store') }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="box-body">
                            ...
                            @if (session('alert-success'))
                                <div class="alert alert-success alert-dismissible">
                                    ...
                                    <h4><i class="icon fa fa-check"></i> File poster diri berhasil diupload!</h4>
                                    Anda masih bisa memperbarui file selama dalam masa periode upload.
                                </div>
                            @endif
                            @if ($isJadwalAvailable)
                                <div class="form-group">
                                    <label for="filePosterDiri">Poster Diri</label>
                                    <input type="file" id="filePosterDiri" name="filePosterDiri">
                            ...
                            @endif
                            @if ($errors->has('filePosterDiri'))
                                <div class="alert alert-danger alert-dismissible">
                                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                    <h4><i class="icon fa fa-ban"></i> Upload Gagal</h4>
                                    {{ $errors->first('filePosterDiri') }}
                                </div>
                            @endif
                            <ul>
