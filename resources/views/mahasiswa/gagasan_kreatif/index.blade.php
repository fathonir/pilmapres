...
@section('content')
    <section class="content-header">
        <h1>Gagasan Kreatif <small>Peserta</small>
        </h1>
        <ol class="breadcrumb">
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="box box-default">
                    <form method="post" action="{{ action('Mahasiswa\GagasanKreatifController@store') }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="box-body">
                            ...
                            @if (session('alert-success'))
                                <div class="alert alert-success alert-dismissible">
                                    ...
                                    <h4><i class="icon fa fa-check"></i> File gagasan kreatif berhasil diupload!</h4>
                                    Anda masih bisa memperbarui file selama dalam masa periode upload.
                                </div>
                            @endif
                            @if ($isJadwalAvailable)
                                <div class="form-group">
                                    <label for="fileGagasanKreatif">Gagasan Kreatif</label>
                                    <input type="file" id="fileGagasanKreatif" name="fileGagasanKreatif">
                            ...
                            @endif
                            @if ($errors->has('fileGagasanKreatif'))
                                <div class="alert alert-danger alert-dismissible">
                                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                    <h4><i class="icon fa fa-ban"></i> Upload Gagal</h4>
                                    {{ $errors->first('fileGagasanKreatif') }}
                                </div>
                            @endif
                            <ul>
